...
class Turret(Subsystem):
    ...
    def __init__(
        ...
        wpilib.SmartDashboard.putData(
            self._nt_base + "/pid", self.controller)
    ...
    def _setup_nt_topics(self) -> None:
        """Create NT publishers and entries for turret telemetry."""
        from ntcore import NetworkTableInstance
        table = NetworkTableInstance.getDefault().getTable(self._nt_base)

        # Read-only telemetry publishers
        self._nt_position = table.getDoubleTopic("position").publish()
        self._nt_velocity = table.getDoubleTopic("velocity").publish()
        self._nt_applied_output = table.getDoubleTopic("appliedOutput").publish()
        self._nt_current = table.getDoubleTopic("current").publish()
        self._nt_bus_voltage = table.getDoubleTopic("busVoltage").publish()
        self._nt_temperature = table.getDoubleTopic("temperature").publish()
        self._nt_target_position = table.getDoubleTopic("targetPosition").publish()
        self._nt_at_target = table.getBooleanTopic("atTargetPosition").publish()
        self._nt_min_soft_limit = table.getDoubleTopic("minSoftLimit").publish()
        self._nt_max_soft_limit = table.getDoubleTopic("maxSoftLimit").publish()
        self._nt_fwd_limit_hit = table.getBooleanTopic("forwardLimitHit").publish()
        self._nt_rev_limit_hit = table.getBooleanTopic("reverseLimitHit").publish()
        self._nt_is_calibrated = table.getBooleanTopic("isCalibrated").publish()
        self._nt_is_calibrating = table.getBooleanTopic("isCalibrating").publish()
        self._nt_is_homing = table.getBooleanTopic("isHoming").publish()

        # Tunable parameters (read-write entries)
        self._nt_min_output_voltage = table.getDoubleTopic("pid/minOutputVoltage").getEntry(self._min_output_voltage)
        self._nt_max_output_voltage = table.getDoubleTopic("pid/maxOutputVoltage").getEntry(self._max_output_voltage)
        self._nt_min_output_voltage.set(self._min_output_voltage)
        self._nt_max_output_voltage.set(self._max_output_voltage)
    ...
    def configureMechanism2d(self) -> None:
        ...
        wpilib.SmartDashboard.putData(
            self._nt_base + "/mechanism", self.mech2d)
    ...
    def updateTelemetry(self) -> None:
        ...
        """
        self._nt_position.set(self.encoder.getPosition())
        self._nt_velocity.set(self.encoder.getVelocity())
        self._nt_applied_output.set(self.motor.getAppliedOutput())
        self._nt_current.set(self.motor.getOutputCurrent())
        self._nt_bus_voltage.set(self.motor.getBusVoltage())
        ...
        self._nt_max_soft_limit.set(sl.getForwardSoftLimit())
        # Limit switches
        self._nt_fwd_limit_hit.set(self.motor.getForwardLimitSwitch().get())
        self._nt_rev_limit_hit.set(self.motor.getReverseLimitSwitch().get())
        # Voltage output limits (read back from NT)
        self._min_output_voltage = self._nt_min_output_voltage.get(self._min_output_voltage)
        self._max_output_voltage = self._nt_max_output_voltage.get(self._max_output_voltage)
        # Calibration status
        self._nt_is_calibrated.set(self.calibration.is_calibrated)
        self._nt_is_calibrating.set(self.calibration.is_calibrating)
        self._nt_is_homing.set(self.calibration.is_homing)
