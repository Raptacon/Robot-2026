...
class SmokeTests(commands2.SequentialCommandGroup):
    ...
    def __init__(self,
        ...
        for index, swerve_module in enumerate(self.drivetrain.swerve_modules):
            if index == 0:
                self.addCommands(
                    commands2.cmd.runOnce(lambda index=index, swerve_module=swerve_module:self.setMessage(0)),
            ...
            self.addCommands(
                ...
                commands2.InstantCommand(lambda swerve_module=swerve_module: swerve_module.set_state(SwerveModuleState(0.2, Rotation2d.fromDegrees(0)), apply_cosine_scaling=False),
                                        self.drivetrain),
                commands2.cmd.runOnce(lambda index=index, swerve_module=swerve_module:self.setMessage((index*5)+1, str(swerve_module.getName()), "Module starts driving")),
                commands2.WaitUntilCommand(lambda: self.progress).finallyDo(lambda _: self.advance(False)),
                ...
                commands2.InstantCommand(lambda swerve_module=swerve_module: swerve_module.set_state(SwerveModuleState(0, Rotation2d.fromDegrees(0)), apply_cosine_scaling=False),
                                        self.drivetrain),
                commands2.cmd.runOnce(lambda index=index, swerve_module=swerve_module:self.setMessage((index*5)+2, str(swerve_module.getName()), "Module stops driving")),
                commands2.WaitUntilCommand(lambda: self.progress).finallyDo(lambda _: self.advance(False)),
                # Current swerve module's steer motor rotates to 0 degrees and driver confirms
                commands2.InstantCommand(lambda swerve_module=swerve_module: swerve_module.set_state(SwerveModuleState(0, Rotation2d.fromDegrees(0)), apply_cosine_scaling=False),
                                        self.drivetrain),
                commands2.cmd.runOnce(lambda index=index, swerve_module=swerve_module:self.setMessage((index*5)+3, str(swerve_module.getName()), "Module rotates facing 0 degrees")),
                commands2.WaitUntilCommand(lambda: self.progress).finallyDo(lambda _: self.advance(False)),
                ...
                commands2.InstantCommand(lambda swerve_module=swerve_module: swerve_module.set_state(SwerveModuleState(0, Rotation2d.fromDegrees(45)), apply_cosine_scaling=False),
                                        self.drivetrain),
                commands2.cmd.runOnce(lambda index=index, swerve_module=swerve_module:self.setMessage((index*5)+4, str(swerve_module.getName()), "Module rotates facing 45 degrees")),
                commands2.WaitUntilCommand(lambda: self.progress).finallyDo(lambda _: self.advance(False)),
                ...
                commands2.InstantCommand(lambda swerve_module=swerve_module: swerve_module.set_state(SwerveModuleState(0, Rotation2d.fromDegrees(90)), apply_cosine_scaling=False),
                                        self.drivetrain),
                commands2.cmd.runOnce(lambda index=index, swerve_module=swerve_module:self.setMessage((index*5)+5, str(swerve_module.getName()), "Module rotates facing 90 degrees")),
                commands2.WaitUntilCommand(lambda: self.progress).finallyDo(lambda _: self.advance(False)),
