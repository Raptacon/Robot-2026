...
class PositionCalibration:
    ...
    **Optional** (enhance safety/features but not required):
    ...
    | `set_conversion_factor` | `(float) -> None` | Apply a new encoder position conversion factor |
    ...
    def homing_init(
        ...
        Args:
            ...
            home_position: encoder value to set when the hard stop is
                found (default 0.0)
            max_travel: abort if encoder travels more than this many user
                units from the start position. Catches a stuck-open limit
                switch that never triggers. None disables the check —
                useful when the mechanical range is unknown. Requires
                ``get_position`` callback to take effect.
            min_trigger_travel: minimum encoder travel (user units)
                required before a limit switch trigger is trusted. If
                the switch fires before this much movement, abort with
                a stuck-closed-switch alert. 0.0 disables (default).
                Requires ``get_position`` callback to take effect.
        """
        ...
        # Capture start position for travel tracking. Only meaningful if
        # get_position is wired; otherwise both travel-based guards are
        # silently no-ops.
    ...
    def calibration_init(
        ...
        Args:
            ...
            max_travel: abort a homing phase if encoder travels more
                than this from its phase start. Defaults to 110% of the
                current soft-limit range (a sane bound for calibration
                because the expected range is known). Pass an explicit
                None via the kwarg to disable the check entirely.
        ...
        # Default max_travel to 110% of the expected range. Each phase
        # homes one direction, so the full range is the per-phase ceiling.
        if max_travel is None:
            expected_range = abs(self._max_soft_limit - self._min_soft_limit)
            if expected_range > 0:
                max_travel = expected_range * 1.1
        ...
        self._cal_max_travel = max_travel
    ...
    def compute_conversion_factor(
        ...
        method computes the conversion factor needed so the measured
        range maps to ``[desired_min, desired_max]``.

        Example: if calibration measured 500 user-units of travel with
        the initial conversion factor and you want -90..90 (180°), the
        corrected factor scales the encoder so full travel reads 180°.

        Args:
            current_factor: the position conversion factor currently set
                on the encoder
            desired_min: target minimum position in user units
            desired_max: target maximum position in user units

        Returns:
            The corrected conversion factor to apply to the encoder.

        Raises:
            RuntimeError: if calibration has not completed, or the
                measured range is zero.
        """
        if not self._is_calibrated:
            raise RuntimeError(
    ...
    def apply_conversion_factor(
        ...
        Calls :meth:`compute_conversion_factor` for the new value, then
        applies it via the ``set_conversion_factor`` callback, resets
        the encoder to ``desired_min`` (the new zero reference), and
        rewrites the internal hard/soft limits to match the desired
        range.

        This is intended to be called immediately after a full
        calibration completes (encoder at the reverse hard stop,
        reading 0). A guard rejects the call if the encoder reports
        more than 5% of the current soft-limit range away from zero —
        the mechanism has moved since calibration, so the post-rescale
        encoder zero would not correspond to a known physical position.

        Args:
            current_factor: the position conversion factor currently set
                on the encoder
            desired_min: target minimum position in user units (becomes
        ...
        Raises:
            ValueError: if the ``set_conversion_factor`` callback is
                not configured.
            RuntimeError: if calibration has not produced a zero
                reference, or the mechanism is not at that zero point.
        """
        ...
        # Verify the mechanism is still at the reverse hard stop
        # (encoder reads ~0) before rescaling. Without this, the post-
        # rescale encoder zero corresponds to no known physical point.
        if self._cb_get_position is not None:
        ...
        # The encoder unit-scale just changed, so its raw reading no
        # longer maps to the same physical position. Reseed to
        # desired_min (we calibrated from the reverse hard stop) and
        # rewrite limits to match the desired range.
    ...
    def _homing_periodic(self) -> bool:
        ...
        # Catches a stuck-open limit switch that never triggers and a
        # broken stall-detection path. Requires get_position to take
        # effect; otherwise it's a silent no-op.
        if (self._max_travel is not None
            ...
            if travel > self._max_travel:
                self._homing_error_alert.setText(
                    ...
                    f"({travel:.1f} > {self._max_travel:.1f})"
                )
                self._homing_end(abort=True)
                return True
        ...
        if limit_hit:
            # Stuck-closed guard: if the switch fires before we've moved
            # enough to be credible, treat it as a false trigger.
                ...
                if self._homing_succeeded:
                    ...
                    if self._cal_known_range is not None:
                        ...
                        self.homing_init(
                            ...
                            max_travel=self._cal_max_travel,
                            min_trigger_travel=(
                                self._cal_min_trigger_travel),
                        )
                else:
